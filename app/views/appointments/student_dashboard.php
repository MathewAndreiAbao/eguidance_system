...
<div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
    ...
    <form method="GET" action="<?= site_url('appointments/student_dashboard') ?>" class="bg-white shadow rounded-lg p-4 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Search</label>
                <input type="text" name="search" value="<?= html_escape($filters['search'] ?? '') ?>" placeholder="Search by counselor, purpose, status..." class="w-full border border-gray-300 rounded-md px-3 py-2 focus:ring-primary focus:border-primary">
            </div>
            <div>
        ...
        <div class="mt-4 flex items-center space-x-3">
            ...
            <a href="<?= site_url('appointments/student_dashboard') ?>" class="text-sm text-gray-600 hover:text-gray-900">Reset filters</a>
        </div>
    </form>

    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
        <div class="px-4 py-5 sm:px-6 flex justify-between items-center">
            <h2 class="text-lg font-medium text-gray-900">My Appointments</h2>
            <div class="flex space-x-2">
                <a href="<?= site_url('appointments/create') ?>" class="bg-primary hover:bg-secondary text-white font-bold py-2 px-4 rounded">
        ...
        <div class="border-t border-gray-200">
            <?php if (empty($appointments)): ?>
                <div class="px-6 py-8 text-center">
                    ...
                    <a href="<?= site_url('appointments/create') ?>" class="mt-4 inline-block bg-primary hover:bg-secondary text-white font-medium py-2 px-4 rounded">
                    ...
                    <?php foreach ($appointments as $appointment): ?>
                        <div class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                            <div class="flex justify-between items-start">
                                <div>
                                    <h3 class="font-medium text-gray-900"><?= html_escape($appointment['counselor_name'] ?? 'Unknown Counselor') ?></h3>
                            ...
                            <div class="mt-4 flex space-x-2">
                                <?php if ($appointment['status'] == 'pending'): ?>
                                    <a href="<?= site_url('appointments/edit/' . $appointment['id']) ?>" class="text-sm bg-primary hover:bg-secondary text-white font-medium py-1 px-3 rounded">Edit</a>
